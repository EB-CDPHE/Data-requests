/**********************************************************************************************
PROGRAM:  RFI.HH_transmission_Part3.sas
AUTHOR:   Eric Bush
CREATED:  November 10, 2021
MODIFIED: February 4, 2022	
PURPOSE:	 re-do HH transmission for start of Omicron in CO (19DEC21)
INPUT:	 	  
OUTPUT:		
***********************************************************************************************/
...
DATA CEDRS_filtered2;  set COVID.CEDRS_view_fix;
   if CountyAssigned ^= 'INTERNATIONAL'  AND
      ('19DEC21'd le  ReportedDate  le '31JAN22'd )  ;
...
DATA CEDRS_DEC21;  set COVID.CEDRS_view_fix;
   if CountyAssigned ^= 'INTERNATIONAL'  AND
      ('19DEC21'd le  ReportedDate  le '31DEC21'd )  ;
...
/*--------------------------------------------------------------------*
 |FINDINGS:
 | n=136 obs missing data for Address1 and Address2
 | N=11 obs where Address1='' and Address2 contains data. THEREFORE:
...
/*----------------------------------------------------------------------------------*
 |FINDINGS:
 | n=75 obs missing data for Address_City, Address_CityActual, and AddressActual
 | n=29 of these are in CO.  n=3 have data in Address1
 *----------------------------------------------------------------------------------*/
...
/*----------------------------------------------*
 ...
 | State = missing for n=875 records 
 *----------------------------------------------*/
 ...
 * Colorado Records with full address (address1, city, state, county) *;
   PROC freq data= CEDRS_DEC21  order=freq;
...
* clean Address1 data *;
   ...
   if ProfileID= '2058981' then do;  tmp_Address1=Address1; Address1=Address2; Address2= tmp_Address1;  end;
...
/*---------------------------------------------------------------------------------*
 |FINDINGS:
 |  N=188,184 filtered cases from Colorado with Address, City, and County data
 *---------------------------------------------------------------------------------*/
