/**********************************************************************************************
PROGRAM:  RFI.COPHS_rates_MONT_API.sas
AUTHOR:   Eric Bush
...
***----------------------------------------------------------------------***;
   ...
   %Let Grp_population = 219 ;      * <-- put population here **;


DATA COPHS_CY21; set COVID.COPHS_fix; 
   where Hosp_Admission ge '01JAN21'd   AND  Race in ('Asian', 'Pacific Islander/Native Hawaiian')  AND  county_of_residence = 'MONTEZUMA';
...
*add vars to describe population (which will be missing for obs from Timeline only) *;
      county_of_residence='MONTEZUMA';  Race_Ethnic='Asian / Pacific Islander';
run;


**  Calculate 7-day moving averages  **;
   PROC expand data=COPHS_CY21_dates   out=MONT_API_MovingAverage  method=none;
...
run;
   ...
   PROC contents data=MONT_API_MovingAverage varnum ;  title1 'MONT_API_MovingAverage';  run;
...
run;
   ...
   Proc Print data= MONT_API_MovingAverage;
      where NumHosp_perDay>0;
run;
