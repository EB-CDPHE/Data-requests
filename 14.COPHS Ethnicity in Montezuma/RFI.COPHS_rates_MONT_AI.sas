/**********************************************************************************************
PROGRAM:  RFI.COPHS_rates_MONT_AI.sas
AUTHOR:   Eric Bush
...
***----------------------------------------------------------------------***;
   ...
   %Let Grp_population = 3991 ;      * <-- put population here **;


DATA COPHS_CY21; set COVID.COPHS_fix; 
   where Hosp_Admission ge '01JAN21'd   AND  Race in ('American Indian/Alaskan Native')  AND  county_of_residence = 'MONTEZUMA';
...
*add vars to describe population (which will be missing for obs from Timeline only) *;
      county_of_residence='MONTEZUMA';  Race_Ethnic='American Indian';
run;


**  Calculate 7-day moving averages  **;
   PROC expand data=COPHS_CY21_dates   out=MONT_AI_MovingAverage  method=none;
...
run;
   ...
   PROC contents data=MONT_AI_MovingAverage varnum ;  title1 'MONT_AI_MovingAverage';  run;
...
run;
   ...
   Proc Print data= MONT_AI_MovingAverage;
