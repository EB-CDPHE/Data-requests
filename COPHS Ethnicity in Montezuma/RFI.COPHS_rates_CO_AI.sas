/**********************************************************************************************
PROGRAM:  RFI.COPHS_rates_CO_AI.sas
AUTHOR:   Eric Bush
...
***----------------------------------------------------------------------***;
   ...
   %Let Grp_population = 111840 ;      * <-- put population here **;


DATA COPHS_CY21; set COVID.COPHS_fix; 
   where Hosp_Admission ge '01JAN21'd   AND  Race = 'American Indian/Alaskan Native';
...
*add vars to describe population (which will be missing for obs from Timeline only) *;
      county_of_residence='ALL counties';  Race_Ethnic='American Indian';
run;


**  Calculate 7-day moving averages  **;
   PROC expand data=COPHS_CY21_dates   out=CO_AI_MovingAverage  method=none;
...
run;
   ...
   PROC contents data=CO_AI_MovingAverage varnum ;  title1 'CO_AI_MovingAverage';  run;
